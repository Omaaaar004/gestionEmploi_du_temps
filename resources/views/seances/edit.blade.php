...
<div class="card">
    ...
    <form action="{{ route('seances.update', $seance->id) }}" method="POST">
        ...
        <div class="form-group">
            ...
            <select name="jour" required>
                ...
                @foreach(['Lundi','Mardi','Mercredi','Jeudi','Vendredi','Samedi'] as $jour)
                    <option value="{{ $jour }}" {{ (old('jour') ?? $seance->jour) == $jour ? 'selected' : '' }}>{{ $jour }}</option>
                @endforeach
            </select>
        ...
        <div class="form-group">
            ...
            <input type="time" name="heure_deb" value="{{ old('heure_deb') ?? $seance->heure_deb }}" required>
        ...
        <div class="form-group">
            ...
            <select id="filiere-select" name="filiere_id" required>
                ...
                @foreach($filieres as $filiere)
                    <option value="{{ $filiere->id }}" {{ (old('filiere_id') ?? $seance->filiere_id) == $filiere->id ? 'selected' : '' }}>{{ $filiere->nom }}</option>
                @endforeach
            </select>
        ...
        <div class="form-group">
            ...
            <select name="prof_id" required>
                ...
                @foreach($profs as $prof)
                    <option value="{{ $prof->id }}" {{ (old('prof_id') ?? $seance->prof_id) == $prof->id ? 'selected' : '' }}>{{ $prof->nom }} {{ $prof->prenom }}</option>
                @endforeach
            </select>
        ...
        <div class="form-group">
            ...
            <select name="local_id">
                ...
                @foreach($locals as $local)
                    <option value="{{ $local->id }}" {{ (old('local_id') ?? $seance->local_id) == $local->id ? 'selected' : '' }}>{{ $local->nom_local }}</option>
                @endforeach
            </select>
